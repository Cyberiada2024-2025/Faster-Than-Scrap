...
@export var START_SPEED_RANGE : float = 5.0
@export var START_SIZE_RANGE : float = 5.0
...
func _on_body_entered(body: Node) -> void:
	print('hit')
	body.take_damage(self.scale.x * self.linear_velocity.length() * damage_multiplier)
